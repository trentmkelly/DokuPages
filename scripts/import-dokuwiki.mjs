...
set document_count = (
  ...
  for (const media of plan.media) {
    ...
    const revisionChange = mediaRevisionImportMetadata(mediaChangesByRevision.get(revisionId), {
      summary: "Imported from DokuWiki media files",
      changeType: "create"
    });
...
on conflict(id) do update set
  ...
  for (const revision of plan.mediaRevisions) {
    ...
    const revisionChange = mediaRevisionImportMetadata(mediaChangesByRevision.get(revisionId), {
      summary: "Imported from DokuWiki media attic",
      changeType: "edit"
    });
  ...
  ${sql(revisionChange.authorId)}, ${sql(revisionChange.summary)}, ${sql(
    revisionChange.changeType
  )}, ${sql(createdAt)}
);`
    );
  }
...
function mediaRevisionImportMetadata(change, fallback) {
  const metadata = revisionImportMetadata(change, { ...fallback, sizeChange: 0 });
  return {
    ...metadata,
    changeType: metadata.changeType === "minor" ? "edit" : metadata.changeType
  };
}

...
and configuration management for admin users, and the media manager. Admin users
can trigger a search index rebuild from the dashboard and purge the rendered
cache. Manager users can view the dashboard but cannot edit ACL rules, manage
users, inspect audit logs, inspect/export configuration, run rebuild actions, or
purge caches unless they also belong to the `admin` group.

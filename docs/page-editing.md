...
`GET /wiki/:id?do=edit` acquires a Durable Object-backed page lock for the configured `LOCKTIME` duration and stores the token in a hidden form field plus an HTTP-only page lock cookie. A second editor receives the DokuWiki-style locked page with HTTP 423 until the lock is released or expires. Autosave refreshes the lock with `POST /api/pages/lock`; the client refresh interval is derived from `LOCKTIME - 60` seconds like upstream DokuWiki's lock timer. Draft autosave follows upstream's 30-second edit refresh cadence when `USEDRAFT` is enabled, and `GET /wiki/:id?do=draft` renders the DokuWiki-style draft recovery decision page with a diff plus recover/delete/cancel actions. Successful saves or draft deletion release the lock. The edit form also carries DokuWiki's `sectok` field for logged-in saves, drafts, and lock refresh/release calls; anonymous edits carry an empty `sectok` like upstream.

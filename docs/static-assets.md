...
`public/dokuwiki.css` is generated by `npm run style:build`. The generator
compiles the stylesheet list from upstream
`../dokuwiki/lib/tpl/dokuwiki/style.ini`, rewrites DokuWiki image URLs to
Pages-served `/images/*` paths, copies referenced upstream image assets when
missing, and appends `src/styles/dokuwiki-pages-overrides.css`. Use
`npm run style:check` to verify the checked-in CSS still matches the upstream

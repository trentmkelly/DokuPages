...
Native login uses D1-backed users with the native password hash format.
Successful logins create a random session token, store only its SHA-256 hash in
D1, and issue an HTTP-only session cookie. Logout deletes the session row and
clears the cookie. Session, page edit lock, CSRF fallback, flash, breadcrumb,
and frontend prefs cookies follow the DokuWiki-compatible
`COOKIE_DIR`/`cookiedir` path, or the normalized `BASE_DIR` path when
`cookiedir` is blank. They also follow `SECURECOOKIE`/`securecookie` and
`SAMESITECOOKIE`/`samesitecookie`; defaults are DokuWiki's HTTPS-only `Secure`
flag and `SameSite=Lax`. Disabled user rows are rejected during login and when
existing session cookies are resolved.
...
regenerating the token, then redirecting back to the profile page. For native
sessions that `sectok` is the upstream-style HMAC-MD5 of session id plus
username signed with `DOKUWIKI_COOKIE_SALT`; the native CSRF cookie remains a
fallback for non-session identity modes. Tokens use the same JWT-shaped HS256
format as upstream DokuWiki with `iss=dokuwiki`, `sub=<username>`, and
...
files. The shell also uses upstream strings for the skip link, search, site
tools, user tools, page tools, media manager, sitemap, recent changes, and
page-action labels. `WIKI_LANG` selects the localized copy with English
fallback. Regenerate the native TypeScript resource with `npm run lang:auth`
after updating the upstream DokuWiki checkout.

...
`search_terms` and `search_postings` model a D1-backed inverted index. Terms store DokuWiki-style index word lengths beside document counts, replacing the upstream length-sharded `data/index/w*.idx` files with a queryable D1 column. The tokenizer follows DokuWiki's `inc/Search/Indexer.php` character handling closely: Unicode letters and numbers are preserved, Asian word characters are separated before splitting, soft hyphens are removed, special characters including `._-:` split terms, one-character numeric terms are indexed, and bundled `inc/lang/<lang>/stopwords.txt` files are selected from `WIKI_LANG`. Query evaluation follows DokuWiki's default AND semantics, lower-precedence OR, unary exclusions, quoted phrase checks, namespace include/exclude filters, edge wildcard terms, plus `SEARCH_NSLIMIT` and `SEARCH_FRAGMENT` query adjustments for searches launched outside the search page form. Title matches are weighted above body matches, page IDs contribute lookup terms, equal scores use DokuWiki-like namespace-depth then page-id ordering, and postings update during flat-file import plus page save and delete operations. Explicit legacy task-runner and admin index rebuilds use `metadata('config', 'locks', 'indexer')` as a five-minute D1 lease equivalent to upstream `_indexer.lock`; page saves keep their per-page index update in the same D1 batch as the saved revision.
